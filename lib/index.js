...
class TidyCleaner {
     ...
     */
    config = new config_1.TidyConfig();
    /**
    ...
    clean(_url, allowReclean = true) {
        ...
        for (const key of to_remove) {
            if (data.info.allowed.has(key))
                continue;
            if (cleaner.has(key)) {
                data.info.removed.push({ key, value: cleaner.get(key) });
                cleaner.delete(key);
            }
        }
        ...
        if (this.config.allowRedirects) {
            for (const rule of data.info.match) {
                ...
                let value = cleaner_ci.get(target);
                // Sometimes the parameter is encoded
                const isEncoded = (0, utils_1.decodeURL)(value, interface_1.EEncoding.urlc);
        ...
        for (const rule of data.info.match) {
            try {
                ...
                let decoded = (0, utils_1.decodeURL)(encodedString, encoding);
                let target = '';
                ...
                if ((0, utils_1.isJSON)(decoded)) {
                    ...
                    const handler = handlers_1.handlers[rule.decode.handler];
        ...
        const diff = (0, utils_1.getLinkDiff)(data.url, url);
...
const clean = (url) => exports.TidyURL.clean(url);
exports.clean = clean;
