...
var TidyCleaner = /** @class */ (function () {
    ...
    TidyCleaner.prototype.clean = function (_url, allowReclean) {
        ...
        for (var _e = 0, _f = data.info.match; _e < _f.length; _e++) {
            var rule = _f[_e];
            for (var _g = 0, _h = rule.exclude; _g < _h.length; _g++) {
                var reg = _h[_g];
                reg.lastIndex = 0;
        ...
        for (var _k = 0, _l = data.info.replace; _k < _l.length; _k++) {
            var key = _l[_k];
        ...
        for (var _t = 0, _u = data.info.match; _t < _u.length; _t++) {
            var rule = _u[_t];
            if (rule.rev)
